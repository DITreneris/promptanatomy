    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 antialiased">
      <nav aria-label="Breadcrumb" className="relative z-10 mb-8 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
        <ol className="flex items-center gap-2">
          ...
          <li aria-hidden>/</li>
          <li className="text-[#0B1320]" aria-current="page">{t('success.breadcrumb')}</li>
        </ol>
      </nav>
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[url('https://grainy-gradients.vercel.app/noise.svg')]"></div>
      <div className="max-w-2xl w-full bg-[#0B1320] rounded-[64px] p-2 shadow-2xl border border-white/5 relative z-10">
        <div className="bg-white rounded-[54px] p-16 md:p-28 text-center shadow-inner">
          ...
          <Link
            to="/"
            className="block w-full min-h-[48px] py-6 bg-[#0B1320] text-white rounded-[24px] font-black uppercase tracking-widest text-sm hover:brightness-125 transition-all shadow-2xl hover:scale-[1.02] active:scale-95 transform text-center focus:outline-none focus-visible:ring-2 focus-visible:ring-[#CFA73A] focus-visible:ring-offset-2"
          >
            {t('common.backToHome')}
          </Link>
        </div>
