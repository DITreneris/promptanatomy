...
- **LP copy (Hero, Kas yra, Navbar, Ecosystem):** tekstai ir vieši skaičiai – `frontend/src/i18n/translations/en.json` / `lt.json`. Po reikšmingų pakeitimų – [CHANGELOG.md](CHANGELOG.md); kanoninę LP būseną (Navbar elgsena, hero bullets, WhatIs stat kortelės, Ecosystem chrome) atnaujinkite [docs/golden-legacy-standard.md](docs/golden-legacy-standard.md) §1–3; prekės ženklo juostą Navbar – [docs/language-guidelines-en-lt.md](docs/language-guidelines-en-lt.md) §1.
- **LP UI polish (dizaino sistema, CTA, tipografija, spacing):** prieš ad-hoc CSS – [docs/design_system_roadmap2.md](docs/design_system_roadmap2.md) (**v1.0 shipped + v1.1 hygiene**) ir [docs/process/design-system-qa.md](docs/process/design-system-qa.md). Navbar/FAQ chrome = golden-legacy §1–3 (ne stale Phase 6). Be shadcn/`@primer/react`/webfonto (ADR-0001). Regresija – golden-legacy + `npm run build` + CI token grep.
...
- **Operatoriaus tier 9/12:** [docs/upgrade-playbook.md](docs/upgrade-playbook.md) + [docs/user-access-tier-registry.md](docs/user-access-tier-registry.md) §3. Po 9→12 grant – **naujas** magic link (`access_tier=12`); LP 12/12 nereiškia, kad senas `/anatomy/` sesija atrakino M10.
