...
|-------|---------|-----------|
| **frontend/package.json** | `"version": "1.5.0"` | Release SOT for SemVer (build, artefaktai). Git tag mirrors this. |
...
**Pastaba:** Navbar versijos ženkliuko nerodo (Variant B). Nėra `APP_VERSION` / `SCRIPT_NAME` `config.js` — Hero neberodo `v1.5_os.sh`.
...
3. **Versijos atnaujinimas:** `frontend/package.json` → `"version": "X.Y.Z"`.
...
Po kiekvieno release – atnaujinti `frontend/package.json` pagal naują versiją (žr. §3). Nėra `APP_VERSION` `config.js`.
