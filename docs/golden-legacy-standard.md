...
- Navbar: `primaryNavItems` priklauso nuo `hasAccess` (perduodamas iš HomePage): bazė – Kas yra Promptų Anatomija / What is Prompt Anatomy (#what-is-prompt-anatomy, `nav.whatIs`), Kainodara / Pricing #pricing; jei `hasAccess === true` – įtraukiamas ir Mokymai mygtukas, kuris kviečia `handleGoToTraining` → `/api/generate-access-link` → magic link. Desktop (`lg+`) rodo `primaryNavItems` + `desktopSecondaryNavItems` (Ecosystem, Methodology, FAQ). Pilnas `secondaryNavItems` (Cloud, Pro, Repo jei config) – mobile drawer (`allNavItems`). Brand (viena eilutė, be versijos/tagline), kalbos LT|EN su navigate į `/lt`/`/en`, CTA. Logo ir „Home“ – locale-aware (`/lt` arba `/en`). Checkout: `/success` primary CTA – `btn-primary-lg`; `/cancel` secondary – `btn-secondary`.
...
- WhatIsPromptAnatomy (section id what-is-prompt-anatomy; h2, value, mid-funnel nuoroda `#pricing`, 6 blokų piliai, 3 stat kortelės: **450+** kuruoti promptai – `whatIs.stat1Number` / `stat1Label`; skaičiai skaitomi screen reader) – po Hero, prieš Methodology.
