...
- **Training app submodule:** `apps/prompt-anatomy` → `DITreneris/inzinerija` (dabartinis pin: `47448fc`; upstream `package.json` gali būti `1.4.4` kol nebump'ins tag). Magic link tier validacija: parent [`api/verify-access.js`](../api/verify-access.js) `VALID_TIERS [3, 6, 9]`; submodulyje – `constants/pricing.ts`.
