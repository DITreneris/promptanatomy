...
3. **Vercel:** auto-deploy iš GitHub `main`. `vercel.json` → `installCommand` = [`scripts/vercel-install.sh`](../scripts/vercel-install.sh). Private `inzinerija`: Production/Preview env **`INZINERIJA_READ_TOKEN`** (fine-grained PAT Contents: Read) — be jo cache gali žaliuoti, bet pin SHA neatsinaujina; su tokenu skriptas išvalo cache-be-`.git` ir klonuoja pin’ą. Do **not** Redeploy without Build Cache unless the token is set. Build log must show submodule checkout **`b6c1a9a`** (or current §4 pin).
