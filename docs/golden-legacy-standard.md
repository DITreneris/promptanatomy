...
- Navbar: `mainNavItems` – Kas yra (#what-is-prompt-anatomy), Ekosistema (#ekosistema); jei `hasAccess === true` – Mokymai (magic link per `handleGoToTraining`). Desktop (`lg+`) rodo tik `mainNavItems`. `drawerOnlyNavItems` (`nav.alreadyHaveAccess` → `#access`, Cloud, Pro, Repo jei config) – papildomai mobile drawer (`allNavItems`). Cloud/Pro `href` + `utm_source=app&utm_medium=navbar_mobile`, `rel="noopener"`; Repo / kitos trečios šalys – `noopener noreferrer`. Brand (viena eilutė, be versijos/tagline), kalbos LT|EN su navigate į `/lt`/`/en`, CTA. Logo ir „Home“ – locale-aware (`/lt` arba `/en`). Checkout: `/success` ir `/cancel` page title = **`h1`**; `/success` primary CTA – `btn-primary-lg`; `/cancel` secondary – `btn-secondary`. Mobile drawer `#mobile-nav` – `aria-hidden` **ir** `inert` kai uždarytas (hamburger lieka `<nav>` viduje, ne drawer).
- Hero (h1, `hero.badge`, `hero.subtitle`, 3 bullet per `HERO_BULLET_KEYS` (rodomi nuo `sm`), vienas primary CTA `hero.cta` → `#pricing` tame pačiame tabe (EN „Choose a plan“, LT „Rinktis planą“); **be** secondary CTA; **be** social proof pill (`hero.socialProof` pašalintas); kodo blokas su typing animacija). `.cloud` / `.pro` outbound lieka Ecosystem / Footer / mobile drawer.
- TrustedBy ([TrustedBy.jsx](../frontend/src/components/TrustedBy.jsx), section id `trusted-by`) — iškart po Hero, prieš WhatIs; **baltas** fonas (dalis logotipų yra JPG be alfa). Antraštė `trustedBy.title` (`text-label-upper`, LT „Komandos, kurias mokėme“ / EN „Teams we've trained“) — **viena** eilutė, be paaiškinimo pastraipos. **6** logotipai fiksuota tvarka: Pigu.lt, Rizikos Cesija, Apvalaus Kvadrato Mokykla, Cgates, Senoji Baldinė, Hitus Legal (`LOGOS` konstanta, po vieną eilutėje — pašalinimas prieštaravus = vienos eilutės trynimas). Statiški ir mono (`grayscale opacity-70`), **be** marquee ir **be** hover spalvos; aukščiai pagal proporcijas (`h-6 sm:h-7` / `h-8 sm:h-9` / `h-10 sm:h-11`), tarpai `gap-x-4 sm:gap-x-6 md:gap-x-8 lg:gap-x-10` (`flex-wrap` duoda 6 vienoje eilutėje ≥`md` ir 3+3 mobilėje — `md:gap-x-10` 768 px lūžta 5+1). Kiekvienas `<img>` su `width`/`height` (CLS), `loading="lazy"`, `alt` = įmonės pavadinimas. Rodomi tik logotipai su savininko sutikimu (**žodinis** patvirtinimas 2026-09-14 visiems šešiems; 2026-04-25 raštiško el. pašto sąlyga — savininko override). Regeneravimas po `SKIP`/atšaukimo **trina** senus `public/logo/*.webp`, kurių nėra šio paleidimo rinkinyje. Turtas: **tik** generuoti `frontend/public/logo/*.webp` (6 failai); originalai (`.png`/`.jpg`) gyvena **už `public/`** — `frontend/logos-src/`, tad į `dist/` nekeliauja. Regeneravimas: [normalize-logos.mjs](../frontend/scripts/normalize-logos.mjs) (`logos-src` → `public/logo`, `sharp` per `npx`, ne projekto priklausomybė); `SKIP` sąrašas laiko nepatvirtintus šaltinius (pvz. `bagfactory`) nepublikuotus. Skripto output dimensijos turi atitikti `width`/`height` `LOGOS` masyve. **Jokio** `aggregateRating` / `Review` JSON-LD aplink juostą.
- WhatIsPromptAnatomy (section id what-is-prompt-anatomy; h2, `whatIs.valueLine2` – 6-block methodology + 500+ library + GPT/agents guide; be „interaktyvių“ ir be „atsitiktin*“ echo (tik Hero subtitle), mid-funnel nuoroda `#pricing`, 6 blokų piliai, 3 stat kortelės: **500+** šablonai ir promptai (`stat1`); **40+** interaktyvūs įrankiai (`stat2`); **300+** interaktyvios skaidrės (`stat3`); skaičiai skaitomi screen reader) – po Hero, prieš Methodology. `footer.tagline` — trumpas WhatIs variantas **be** brand echo (`whatIs.valueLine1` palieka brandą sakinyje). Prieigos forma – `#access` kortelė `#pricing` sekcijoje; Check mygtukas = `btn-dark` (navy, ne `btn-primary`); viena instrukcija (`pricing.returningBody` – nuoroda į mokymus atsidaro čia, ne el. paštu; lauko etiketė `checkAccess` = el. paštas; be `accessHow` / `accessStepsHint`). FAQ – no `faq.sectionLabel` key / no eyebrow (tik h2); **9** klausimai (`faq.items`), gilesni atsakymai (~60–90 ž.) su hub/spoke kontekstu (įsk. lifetime access); Six-Block vs five-part/Quick mode — **ne** FAQ/WhatIs, tik GEO (`llms.txt` / `TRAINING_SUMMARY`); `<details>` accordion + FAQPage JSON-LD iš to paties i18n.
- Methodology (section id metodologija; dekoratyvus label `<p>` `text-amber-800`; pagrindinis pavadinimas `<h2>`; kortelių antraštės `<h3>`; pull-quote `methodology.paragraph` — trys redakcinės eilutės EN/LT (`\n`: verdiktas / etiketė / grandinė), `whitespace-pre-line`, `max-w-sm`; be papildomo `mt-20`).
- Pricing (section id pricing, prieigos forma `id="access"`, 2 planai Phase 1; trust eilutė `pricing.refundContact` → `/terms#refunds` — **14 d. = atsakymas** (EN „we reply within 14 days“, ne money-back); komandų blokas `forTeamsBody` – LT „Mokymai komandai — 399 €. 15 min. pokalbis: info@promptanatomy.app. Atsakome per 1 darbo dieną.“ (1 d. d. ≠ grąžinimų atsakymo 14 d.; be piloto / SLA / Telegram); „Eiti į mokymus" mygtukas kviečia `/api/generate-access-link` ir nukreipia tame pačiame lange į training app su magic link – same-tab navigation reikalinga iOS/Safari, kur `window.open` po async dažnai blokuojamas).
- Ecosystem (section id ekosistema, `section-dark-ecosystem`): header tik `ecosystem.title` (be pastraipos, be `ctaPricing`, be `workflowHint`); **6** kortelės (`ecosystem.items`) — cloud, info, space, help, ceo, pro; grid **3+3** (`lg:grid-cols-3`, `gap-8`); kiekviena kortelė — `card-density-dark-premium` (`shadow-ecosystem-card-rest`, `min-h-[200px]`): `title` (`<h3>` `text-lg`) + `outcome` (`line-clamp-2`) + tag pills (`ecosystem-tag-pill`, iki 2) + CTA footer (`mt-auto pt-3`, `aria-label` su `opensInNewTab`, be matomo teksto po CTA); **be** phase eyebrow / hub pill / phase legend; **Enter** — `card-featured-ecosystem` (`border-2`), `badge-premium` (`startHere`, `top-6 right-6`), full-width `min-h-[48px]` `btn-primary` + `shadow-ecosystem-cta`, icon glow; kitos 5 — `card-phase-accent-*` top rim, `btn-ecosystem-secondary` + `ArrowRight`, `shadow-ecosystem-icon-depth`; po grid — `mapLink` (LT „Peržiūrėti ekosistemą“, EN „See the ecosystem“) → `promptanatomy.site/#ecosystem` (be `trustLine`). Spoke `href` renderinamas su `utm_source=app` + `utm_medium=ecosystem_card` (map — `ecosystem_site_map`); `rel="noopener"` be `noreferrer`. Identity URL i18n / geo-manifest lieka pliki. Anchor: `scroll-margin-top`. Fazinės spalvos ant ikonų/rim: Adopt (1), Apply (2), Scale (3) — be žodinių etikečių.
- Footer ([Footer.jsx](frontend/src/components/Footer.jsx)): `pt-14 md:pt-16`, 4 lygios kolonos (`lg:col-span-3`) – Brand (`nav.brandPromptu`/`brandAnatomija`, `footer.tagline` ≠ `whatIs.valueLine1`), Produktas (`footer.whatIs` → #what-is-prompt-anatomy, #metodologija, #ekosistema, #pricing), Mokymai (Mokymai magic link jei `hasAccess`, `footer.alreadyHaveAccess` → `#access`, #faq, `nav.cta`), Tinklas (Telegram, LinkedIn, X su `noopener noreferrer`; Cloud/Pro su PostHog `footer_network`, `utm_source=app&utm_medium=footer_network`, `rel="noopener external nofollow"` be `noreferrer`); legal bar – `copyrightLine1`, mailto, privacy/terms/cookies, `creator`, `formatMailingAddressOneLine()` visur `text-xs`; be `copyrightLine2`. Typeface: OS sans ([ADR-0001](decisions/0001-lp-system-typeface.md)).
...
**i18n:** Visi raktai naudojami iš `lt.json` / `en.json`; nėra hardcoded teksto komponentuose (Hero, WhatIs, Methodology, Ecosystem, Pricing, Footer, Navbar, Success, Cancel, Privacy, Terms). LT – terminas DI, kreipinys **tu**, mygtukai **bendratis**; EN – AI. Tas pats raktas ≠ tas pats sakinys ([language-guidelines-en-lt.md](language-guidelines-en-lt.md)). `whatIs.title` EN — „What is Prompt Anatomy?“; `nav.whatIs` be „?“. EN 6-oji ekosistemos kortelė `title` = **Deploy** (sutampa su `cta`). `pricing.modulesLocked` — moduliai **7–12** (Duomenų analizė, Agentai) **ne** per Stripe (ne viešas Stripe kelias); kelias = komanda / atnaujinimas per `info@promptanatomy.app` (C2). FAQ Starter/Core — tas pats M7–12 kelias; FAQ vis dar **9** klausimai. Ecosystem Use `tags` be `100+` (nekonkuruoja su Hero/WhatIs **500+**).
...
- Hero primary CTA – `href="#pricing"`, tas pats tabas; **nėra** `promptanatomy.cloud` ir **nėra** `target="_blank"` ant Hero mygtuko; šalia CTA – **nieko** (nei antro mygtuko, nei social proof pill); proof = Trusted-by juosta po Hero (375 px: 3+3; ≥`md`: viena eilutė).
...
- **Training app submodule:** `apps/prompt-anatomy` → `DITreneris/inzinerija` (dabartinis pin: **`b6c1a9a` / tag `v1.6.4`** — Tomas GO 2026-09-14; M4/M6 Project-first + DS Table + MON-4 PostHog + M13-BATCH authoring assets shipped under corporate12 build; prior live pin `99d2639` / PR #102. Hub build stays `VITE_MAX_BUILD_MODULE=12` — M13–15 **not** unlocked for learners; corporate15 / tier 15 neaktyvuojami; do **not** chase inzinerija `main` past this tag without a new GO). Production profilis: [`scripts/vercel-build.sh`](../scripts/vercel-build.sh) / CI naudoja `VITE_MAX_BUILD_MODULE=12` ir `npm run build:corporate12` (M1–12 bundle). Training PostHog: Production `VITE_POSTHOG_KEY` (+ optional `VITE_POSTHOG_HOST=https://eu.i.posthog.com`) must be present at **build** time. Magic link tier validacija: parent [`api/verify-access.js`](../api/verify-access.js) / [`generate-access-link.js`](../api/generate-access-link.js) `VALID_TIERS` / `ACCESS_TIER_VALUES` `[3, 6, 9, 12]`; submodulyje – `constants/pricing.ts`.
...
- **Statinių URL po deploy:** `GET https://www.promptanatomy.app/sitemap.xml` → **200** ir galiojantis XML; `robots.txt`, `llms.txt`, **`llms-full.txt`** – taip pat 200. `robots.txt` turi turėti `PerplexityBot`, `OAI-SearchBot`, ir 5 share grupes (`facebookexternalhit` / `Facebot` / `Twitterbot` / `LinkedInBot` / `WhatsApp`) su `Allow: /anatomy/` **prieš** `User-agent: *` (kuris vis dar `Disallow: /anatomy/`). Facebook skaito origin `/robots.txt`, ne `/anatomy/robots.txt`. `llms.txt` — Answer.AI shape: H1 `# Prompt Anatomy`, blockquote `>`, markdown links (`[Home EN](...)`, `[Full LLM index](...)`), `## Optional`. `index.html` – `Person` (#founder Tomas Staniulis) + Organization `sameAs`. Po `en.json` `legal.*` pakeitimo – `cd frontend && npm run build` ir patikrinti, kad `dist/privacy.html` / `dist/terms.html` yra **EN** (generuojami `scripts/generate-legal-static.mjs` tik iš `en.json`; Vercel rewrite duoda juos crawleriams ir full-load). LT legal – tik SPA client-nav `useLocale()`, ne atskiras URL. GEO manifest: `frontend/src/site/geo-manifest.js`; org entity: `frontend/src/site/organization.js`; build: `scripts/generate-geo-static.mjs`.
...
5. **GitHub Actions:** pull request ir push į `main` paleidžia [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) (job **Golden Legacy**): `node --test api/lib/sentry.test.js` (be root `npm ci`); `frontend` — `npm ci` + `npm run build`; `apps/prompt-anatomy` — `npm ci` + `npm run build:corporate12` su `VITE_BASE_PATH=/anatomy/`, `VITE_MAX_BUILD_MODULE=12`, `HUSKY=0` (submodulio husky nevykdomas CI); `backend` — `pip install -r requirements.txt` + `pytest`. Tame pačiame job: design-system token grep (LP JSX be `rgba(` / `text-[NNpx]` / `shadow-[`); bundle size budget (`scripts/check-bundle-size.mjs`); GEO smoke (`dist/llms.txt`, `llms-full.txt`, `robots.txt` — PerplexityBot + OAI-SearchBot + 5 share `Allow: /anatomy/` + `*` `Disallow: /anatomy/`). Submoduliai: `actions/checkout` su `submodules: recursive`.
