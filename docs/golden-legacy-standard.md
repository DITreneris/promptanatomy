...
- **Training app submodule:** `apps/prompt-anatomy` → `DITreneris/inzinerija` (dabartinis pin: `a95b2fa`, tag `v1.4.1`). Magic link tier validacija: parent [`api/verify-access.js`](../api/verify-access.js) `VALID_TIERS [3, 6, 9]`; submodulyje – `constants/pricing.ts`.
