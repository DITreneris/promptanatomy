...
- Ecosystem (section id ekosistema): **6** kortelės (`ecosystem.items`) — cloud, info, space, help, ceo, pro; grid **3+3** (`lg:grid-cols-3`, dvi eilutės); **Hub core** — `ecosystem.hubCoreLabel` pill + phase legend (Adopt/Apply/Scale) + desktop connector linijos į korteles; kiekviena kortelė — scan card: `title` + `outcome` (viena naudos eilutė) + `tags` (dot-separated, be bulletų) + per-item `cta` (`btn-ecosystem-outline`, `mt-auto`, `aria-label`); antrinė nuoroda `ecosystem.mapLink` → `promptanatomy.site/#ecosystem` (PostHog `ecosystem_site_map`). Fazinės spalvos: Adopt (1), Apply (2), Scale (3).
...
- **Ecosystem CTA:** Per-item `cta` kiekvienoje kortelėje (LT: Pradėti / Naudoti / Kurti / Atrinkti / Valdyti / Diegti; EN: Start / Use / Create / Hire / Manage / Deploy); **neviršyti ~12 simb.** ant mobilės. `ecosystem.ctaOpen` — tik fallback senam JSON. `ecosystem.mapLink` — antrinė nuoroda po grid (ne primary CTA).
