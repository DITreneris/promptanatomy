...
**i18n:** Visi raktai naudojami iš `lt.json` / `en.json`; nėra hardcoded teksto komponentuose (Hero, WhatIs, Methodology, Ecosystem, Pricing, Footer, Navbar, Success, Cancel, Privacy, Terms). LT – terminas DI, kreipinys **tu**, mygtukai **bendratis**; EN – AI. Tas pats raktas ≠ tas pats sakinys ([language-guidelines-en-lt.md](language-guidelines-en-lt.md)). `whatIs.title` EN — „What is Prompt Anatomy?“; `nav.whatIs` be „?“. EN 6-oji ekosistemos kortelė `title` = **Deploy** (sutampa su `cta`). `pricing.modulesLocked` — moduliai **7–12** pagal poreikį (ne 15): Duomenų analizė, Agentai (be Turinio inžinerijos — tai M13–15, ne Phase-1 checkout). FAQ item 1/3: **500+** = ekosistemos biblioteka (nemokami spokes), ne `/anatomy/` unlock. Ecosystem Use `tags` be `100+` (nekonkuruoja su Hero/WhatIs **500+**).
