...
- Hero (h1, `hero.badge`, `hero.subtitle`, 3 bullet per `HERO_BULLET_KEYS` (rodomi nuo `sm`), CTA `hero.cta` = pricing intent scroll į `#pricing`; **600+** `hero.socialProof`; kodo blokas su typing animacija).
- WhatIsPromptAnatomy (section id what-is-prompt-anatomy; h2, `whatIs.valueLine2` – Six-Block kanonas + Quick mode mapping; be „interaktyvių“ ir be „atsitiktin*“ echo (tik Hero subtitle), mid-funnel nuoroda `#pricing`, 6 blokų piliai, 3 stat kortelės: **500+** šablonai ir promptai – `whatIs.stat1Number` / `stat1Label`; skaičiai skaitomi screen reader) – po Hero, prieš Methodology. `footer.tagline` sutapatintas su `whatIs.valueLine1`. Prieigos forma – `#access` kortelė `#pricing` sekcijoje; viena instrukcija (`pricing.accessHow` – magic link naršyklėje, ne el. paštu; be `accessStepsHint` eyebrow). FAQ – be `faq.sectionLabel` eyebrow (tik h2); **9** klausimai (`faq.items`), įsk. Six-Block vs five-part/Quick mode (pakeitė buvusį „Is access temporary?“); gilesni atsakymai (~60–90 ž.) su hub/spoke kontekstu; `<details>` accordion + FAQPage JSON-LD iš to paties i18n.
