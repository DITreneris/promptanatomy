...
- **Training app submodule:** `apps/prompt-anatomy` → `DITreneris/inzinerija` (dabartinis pin: `1eaa2be`; upstream package `1.4.6` + Unreleased post-1.4.6; tag `v1.4.6` @ `4598402`). Magic link tier validacija: parent [`api/verify-access.js`](../api/verify-access.js) `VALID_TIERS [3, 6, 9]`; submodulyje – `constants/pricing.ts`.
