...
- Ecosystem (section id ekosistema): **6** kortelės (`ecosystem.items`) — cloud, info, space, help, ceo, pro; grid **3+3** (`lg:grid-cols-3`, dvi eilutės); kiekviena kortelė rodo `count`, outcome, 2 bullets, CTA `ecosystem.ctaOpen`; antrinė nuoroda `ecosystem.mapLink` → `promptanatomy.site/#ecosystem` (PostHog `ecosystem_site_map`). Fazinės spalvos: Adopt (1), Apply (2), Scale (3).
