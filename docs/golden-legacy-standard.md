...
- Ecosystem (section id ekosistema): **6** kortelės (`ecosystem.items`) — cloud, info, space, help, ceo, pro; grid **3+3** (`lg:grid-cols-3`, dvi eilutės); **Hub core** — `ecosystem.hubCoreLabel` pill + phase legend (Adopt/Apply/Scale) + desktop connector linijos į korteles; kiekviena kortelė rodo `count`, outcome, 2 bullets, CTA `ecosystem.ctaOpen` (`btn-ecosystem-outline`, `mt-auto`); antrinė nuoroda `ecosystem.mapLink` → `promptanatomy.site/#ecosystem` (PostHog `ecosystem_site_map`). Fazinės spalvos: Adopt (1), Apply (2), Scale (3).
...
- **Ecosystem CTA:** Vieningas `ecosystem.ctaOpen` („View system“ / „Peržiūrėti sistemą“) ant visų 6 kortelių; **neviršyti ~18 simb.** ant mobilės. `ecosystem.mapLink` — antrinė nuoroda po grid (ne primary CTA).
