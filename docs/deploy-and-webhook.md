...
- **Training app (submodulis):** `apps/prompt-anatomy` → [DITreneris/inzinerija](https://github.com/DITreneris/inzinerija) (commit **`b6c1a9a`** / tag **v1.6.4** — [PR #105](https://github.com/DITreneris/promptanatomy/pull/105); prior live pin `99d2639` MON-4 PostHog). `inzinerija` is **private**. [`scripts/vercel-install.sh`](../scripts/vercel-install.sh): with env **`INZINERIJA_READ_TOKEN`** (fine-grained PAT, Contents: Read on `inzinerija`) refreshes the submodule to the pin SHA; without it, keeps a cached `apps/prompt-anatomy` tree (do not Redeploy without Build Cache unless the token is set). Then `HUSKY=0 npm ci` in the submodule. `buildCommand` builds frontend, then training with `VITE_BASE_PATH=/anatomy/`, `VITE_MAX_BUILD_MODULE=12`, `npm run build:corporate12` (be `VITE_MVP_MODE`), copies into `frontend/dist/anatomy/`. Maršrutas `/anatomy/*`; `/anatomija/*` → **308** į `/anatomy/*`. Parent magic-link tiers `[3, 6, 9, 12]`; Supabase Phase 1 grant `highest_plan=12` → `access_tier=12`. Corporate15 / tier 15 neaktyvuojami. Rekomenduojama Vercel build env: `VITE_PUBLIC_SITE_URL=https://www.promptanatomy.app`; Production: `TRAINING_REDIRECT_BASE=https://www.promptanatomy.app/anatomy`; training PostHog: `VITE_POSTHOG_KEY` (+ optional `VITE_POSTHOG_HOST=https://eu.i.posthog.com`) at **build** time (žr. submodulio `docs/deployment/MON-4_POSTHOG_DEPLOY.md`, `MARKETING_HANDOFF_CHECKLIST.md` §7b + `06_marketingo_memo_corporate12_supabase.md`).
...
**Vercel env (private training submodule):** `INZINERIJA_READ_TOKEN` — same PAT as GitHub Actions secret (Contents: Read on `DITreneris/inzinerija`). Required to ship a new submodule pin; without it install may still succeed from Build Cache but stay on a stale training tree.
...
- **Submodulio build** – įsitikinkite, kad training app buildina lokaliai: `cd apps/prompt-anatomy && VITE_BASE_PATH=/anatomy/ VITE_MAX_BUILD_MODULE=12 npm run build:corporate12` (Windows: nustatykite env kintamuosius atitinkamai arba naudokite cross-env).
...
3. **Submoduliai:** *Settings → Git* – **Include Git Submodules**. Private `inzinerija`: set Production/Preview env **`INZINERIJA_READ_TOKEN`**. Build log: `vercel-install` either clones with token or `using cached apps/prompt-anatomy`; pin SHA **`b6c1a9a`** / `VITE_MAX_BUILD_MODULE=12` / `build:corporate12`. Do not Redeploy without Build Cache unless the token is set.
4. **Husky:** submodulio `npm ci` Vercel'e naudoja `HUSKY=0` (žr. `vercel.json` `installCommand`).
5. **Logai:** Deployments → failed build → Build Logs – ieškokite `validate:schema`, `husky`, `submodule`, `destination path … already exists`, `could not read Username`, `ENOMEM` / timeout. Build/install logika – [`scripts/vercel-build.sh`](../scripts/vercel-build.sh), [`scripts/vercel-install.sh`](../scripts/vercel-install.sh) (Vercel `buildCommand` ≤256 simb.).
...
- [ ] **`/anatomy/`** – puslapis kraunasi, navigacija ir asset’ai; **`/anatomija/`** → 308 į `/anatomy/`.
