...
- **Training app (submodulis):** `apps/prompt-anatomy` → [DITreneris/inzinerija](https://github.com/DITreneris/inzinerija) (commit `7e4c3bf`, upstream package `1.6.3` + Horizon B corporate12 marketing pin). Vercel build metu: `installCommand` inicijuoja submodulį (`git submodule update --init --recursive`, `HUSKY=0` submodulyje), `buildCommand` buildina frontend, po to training app su `VITE_BASE_PATH=/anatomy/`, `VITE_MAX_BUILD_MODULE=12`, `npm run build:corporate12` (be `VITE_MVP_MODE`), rezultatas kopijuojamas į `frontend/dist/anatomy/`. Maršrutas `/anatomy/*` aptarnaujamas iš to katalogo; `/anatomija/*` → **308** į `/anatomy/*`. Parent magic-link tiers `[3, 6, 9, 12]`; Supabase Phase 1 grant `highest_plan=12` → `access_tier=12`. Corporate15 / tier 15 neaktyvuojami. Rekomenduojama Vercel build env: `VITE_PUBLIC_SITE_URL=https://www.promptanatomy.app`; Production: `TRAINING_REDIRECT_BASE=https://www.promptanatomy.app/anatomy` (žr. submodulio `docs/deployment/MARKETING_HANDOFF_CHECKLIST.md` §7b + `06_marketingo_memo_corporate12_supabase.md`).
...
3. **Submoduliai:** *Settings → Git* – įjunkite **Include Git Submodules**. Build log turi rodyti `apps/prompt-anatomy` commit `7e4c3bf` ir `VITE_MAX_BUILD_MODULE=12` / `build:corporate12`.
